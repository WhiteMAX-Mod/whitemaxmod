.class public final Lpca;
.super Lgo;
.source "SourceFile"

# interfaces
.implements Lbmg;
.implements Lg4c;


# static fields
.field public static final synthetic n:I


# instance fields
.field public final d:J

.field public final e:J

.field public final f:J

.field public final g:J

.field public final h:Luu9;

.field public final i:Ljava/util/List;

.field public final j:Ljava/util/List;

.field public final k:Z

.field public final l:Ljava/lang/String;

.field public final m:Ljava/lang/String;


# direct methods
.method public constructor <init>(JJJJJLjava/lang/String;Ljava/lang/String;Luu9;Ljava/util/List;Ljava/util/List;Z)V
    .locals 0

    invoke-direct/range {p0 .. p2}, Lgo;-><init>(J)V

    iput-wide p3, p0, Lpca;->d:J

    iput-wide p5, p0, Lpca;->e:J

    iput-wide p7, p0, Lpca;->f:J

    iput-wide p9, p0, Lpca;->g:J

    iput-object p13, p0, Lpca;->h:Luu9;

    iput-object p14, p0, Lpca;->i:Ljava/util/List;

    iput-object p15, p0, Lpca;->j:Ljava/util/List;

    move/from16 p1, p16

    iput-boolean p1, p0, Lpca;->k:Z

    const-string p1, ""

    if-nez p11, :cond_0

    move-object p11, p1

    :cond_0
    iput-object p11, p0, Lpca;->l:Ljava/lang/String;

    if-nez p12, :cond_1

    move-object p12, p1

    :cond_1
    iput-object p12, p0, Lpca;->m:Ljava/lang/String;

    return-void
.end method


# virtual methods
.method public final d(Lmlg;)V
    .locals 11

    check-cast p1, Lqca;

    iget-object v0, p0, Lgo;->c:Lho;

    const/4 v1, 0x0

    if-eqz v0, :cond_0

    goto :goto_0

    :cond_0
    move-object v0, v1

    :goto_0
    invoke-virtual {v0}, Lho;->h()Lkq9;

    move-result-object v0

    iget-wide v2, p0, Lpca;->e:J

    invoke-virtual {v0, v2, v3}, Lkq9;->n(J)Lmq9;

    move-result-object v8

    if-eqz v8, :cond_1

    iget-object v0, v8, Lmq9;->j:Luu9;

    sget-object v4, Luu9;->c:Luu9;

    if-ne v0, v4, :cond_2

    :cond_1
    :goto_1
    move-object v7, p0

    goto/16 :goto_5

    :cond_2
    iget-object v6, p1, Lqca;->c:Lzn9;

    if-nez v6, :cond_3

    goto :goto_1

    :cond_3
    iget-object p1, p0, Lgo;->c:Lho;

    if-eqz p1, :cond_4

    goto :goto_2

    :cond_4
    move-object p1, v1

    :goto_2
    invoke-virtual {p1}, Lho;->h()Lkq9;

    move-result-object v5

    new-instance v4, Lfs;

    const/4 v9, 0x6

    move-object v7, p0

    invoke-direct/range {v4 .. v9}, Lfs;-><init>(Ljava/lang/Object;Ljava/lang/Object;Ljava/lang/Object;Ljava/lang/Object;I)V

    iget-object p1, v5, Lkq9;->a:Lon4;

    invoke-virtual {p1}, Lon4;->c()Llz9;

    move-result-object p1

    check-cast p1, Lqae;

    invoke-virtual {p1}, Lqae;->g()Lgo4;

    move-result-object p1

    invoke-virtual {p1, v4}, Lgo4;->a(Lzt6;)Ljava/lang/Object;

    iget-object p1, v7, Lgo;->c:Lho;

    if-eqz p1, :cond_5

    goto :goto_3

    :cond_5
    move-object p1, v1

    :goto_3
    invoke-virtual {p1}, Lho;->c()Lmn2;

    move-result-object p1

    iget-wide v4, v7, Lpca;->d:J

    invoke-virtual {p1, v4, v5}, Lmn2;->P(J)Lqk2;

    move-result-object p1

    iget-object v0, v8, Lmq9;->H:Lc05;

    invoke-virtual {v0}, Lc05;->c()Z

    move-result v0

    if-eqz v0, :cond_7

    if-eqz p1, :cond_7

    iget-object p1, p1, Lqk2;->b:Llo2;

    iget-wide v9, p1, Llo2;->j:J

    cmp-long p1, v9, v2

    if-nez p1, :cond_7

    iget-object p1, v7, Lgo;->c:Lho;

    if-eqz p1, :cond_6

    goto :goto_4

    :cond_6
    move-object p1, v1

    :goto_4
    invoke-virtual {p1}, Lho;->c()Lmn2;

    move-result-object p1

    invoke-virtual {p1, v4, v5}, Lmn2;->K(J)V

    :cond_7
    iget-object p1, v7, Lgo;->c:Lho;

    if-eqz p1, :cond_8

    move-object v1, p1

    :cond_8
    invoke-virtual {v1}, Lho;->b()Ln11;

    move-result-object p1

    new-instance v0, Lleh;

    iget-wide v3, v8, Lxm0;->a:J

    const/4 v5, 0x0

    iget-wide v1, v7, Lpca;->d:J

    invoke-direct/range {v0 .. v5}, Lleh;-><init>(JJZ)V

    invoke-virtual {p1, v0}, Ln11;->c(Ljava/lang/Object;)V

    :goto_5
    return-void
.end method

.method public final e()I
    .locals 19

    move-object/from16 v0, p0

    iget-object v1, v0, Lgo;->c:Lho;

    if-eqz v1, :cond_0

    goto :goto_0

    :cond_0
    const/4 v1, 0x0

    :goto_0
    invoke-virtual {v1}, Lho;->h()Lkq9;

    move-result-object v1

    iget-wide v3, v0, Lpca;->e:J

    invoke-virtual {v1, v3, v4}, Lkq9;->n(J)Lmq9;

    move-result-object v1

    iget-object v5, v0, Lgo;->c:Lho;

    if-eqz v5, :cond_1

    goto :goto_1

    :cond_1
    const/4 v5, 0x0

    :goto_1
    invoke-virtual {v5}, Lho;->c()Lmn2;

    move-result-object v5

    iget-wide v6, v0, Lpca;->d:J

    invoke-virtual {v5, v6, v7}, Lmn2;->P(J)Lqk2;

    move-result-object v5

    iget-object v8, v0, Lgo;->c:Lho;

    if-eqz v8, :cond_2

    goto :goto_2

    :cond_2
    const/4 v8, 0x0

    :goto_2
    invoke-virtual {v8}, Lho;->j()Lrng;

    move-result-object v8

    sget-object v9, Lh4c;->i:Lh4c;

    iget-wide v10, v0, Lgo;->a:J

    invoke-virtual {v8, v10, v11, v9}, Lrng;->h(JLh4c;)Ljava/util/List;

    move-result-object v8

    invoke-interface {v8}, Ljava/util/List;->iterator()Ljava/util/Iterator;

    move-result-object v8

    :cond_3
    invoke-interface {v8}, Ljava/util/Iterator;->hasNext()Z

    move-result v9

    const/4 v12, 0x3

    const-string v13, "pca"

    if-eqz v9, :cond_4

    invoke-interface {v8}, Ljava/util/Iterator;->next()Ljava/lang/Object;

    move-result-object v9

    check-cast v9, Leng;

    iget-object v9, v9, Leng;->f:Lg4c;

    check-cast v9, Lpca;

    iget-wide v14, v9, Lpca;->d:J

    cmp-long v14, v14, v6

    if-nez v14, :cond_3

    iget-wide v14, v9, Lpca;->e:J

    cmp-long v9, v14, v3

    if-nez v9, :cond_3

    const-string v1, "onPreExecute: later edit task found, REMOVE"

    invoke-static {v13, v1}, Lq98;->z(Ljava/lang/String;Ljava/lang/String;)V

    return v12

    :cond_4
    if-eqz v1, :cond_12

    iget-object v3, v1, Lmq9;->j:Luu9;

    sget-object v4, Luu9;->c:Luu9;

    if-eq v3, v4, :cond_12

    if-eqz v5, :cond_12

    invoke-virtual {v5}, Lqk2;->S()Z

    move-result v3

    if-nez v3, :cond_5

    invoke-virtual {v5}, Lqk2;->i0()Z

    move-result v3

    if-nez v3, :cond_5

    goto/16 :goto_9

    :cond_5
    iget-wide v3, v0, Lpca;->g:J

    const-wide/16 v5, 0x0

    cmp-long v7, v3, v5

    if-nez v7, :cond_6

    const-string v1, "onPreExecute: message serverId == 0, REMOVE"

    invoke-static {v13, v1}, Lq98;->z(Ljava/lang/String;Ljava/lang/String;)V

    return v12

    :cond_6
    const/4 v7, 0x2

    const-string v8, "onPreExecute: attaches not ready, SKIP"

    iget-boolean v9, v0, Lpca;->k:Z

    if-eqz v9, :cond_10

    sget-object v14, Lh50;->c:Lh50;

    invoke-virtual {v1, v14}, Lmq9;->x(Lh50;)Z

    move-result v14

    if-eqz v14, :cond_10

    iget-object v14, v1, Lmq9;->n:Lc40;

    if-eqz v14, :cond_7

    iget-object v14, v14, Lc40;->a:Ljava/lang/Object;

    check-cast v14, Ljava/util/List;

    goto :goto_3

    :cond_7
    const/4 v14, 0x0

    :goto_3
    if-nez v14, :cond_8

    sget-object v14, Lwm5;->a:Lwm5;

    :cond_8
    invoke-interface {v14}, Ljava/util/List;->iterator()Ljava/util/Iterator;

    move-result-object v14

    :cond_9
    :goto_4
    invoke-interface {v14}, Ljava/util/Iterator;->hasNext()Z

    move-result v15

    if-eqz v15, :cond_10

    invoke-interface {v14}, Ljava/util/Iterator;->next()Ljava/lang/Object;

    move-result-object v15

    check-cast v15, Lm50;

    invoke-virtual {v15}, Lm50;->e()Z

    move-result v16

    if-eqz v16, :cond_9

    iget-object v15, v15, Lm50;->b:Lx40;

    move-wide/from16 v17, v3

    iget-wide v2, v15, Lx40;->i:J

    cmp-long v2, v2, v5

    if-eqz v2, :cond_f

    iget-object v2, v15, Lx40;->h:Ljava/lang/String;

    invoke-static {v2}, Llb4;->z0(Ljava/lang/CharSequence;)Z

    move-result v2

    if-eqz v2, :cond_f

    iget-object v1, v0, Lgo;->c:Lho;

    if-eqz v1, :cond_a

    goto :goto_5

    :cond_a
    const/4 v1, 0x0

    :goto_5
    invoke-virtual {v1}, Lho;->j()Lrng;

    move-result-object v1

    invoke-virtual {v1, v10, v11}, Lrng;->j(J)Leng;

    move-result-object v1

    if-eqz v1, :cond_e

    iget v1, v1, Leng;->c:I

    const/16 v2, 0x14

    if-le v1, v2, :cond_b

    goto :goto_8

    :cond_b
    iget-object v1, v0, Lgo;->c:Lho;

    if-eqz v1, :cond_c

    goto :goto_6

    :cond_c
    const/4 v1, 0x0

    :goto_6
    invoke-virtual {v1}, Lho;->a()Lv2b;

    move-result-object v1

    invoke-static/range {v17 .. v18}, Ljava/lang/Long;->valueOf(J)Ljava/lang/Long;

    move-result-object v2

    invoke-static {v2}, Ljava/util/Collections;->singletonList(Ljava/lang/Object;)Ljava/util/List;

    move-result-object v2

    iget-wide v3, v0, Lpca;->f:J

    invoke-virtual {v1, v3, v4, v2}, Lv2b;->B(JLjava/util/List;)J

    iget-object v1, v0, Lgo;->c:Lho;

    if-eqz v1, :cond_d

    move-object v2, v1

    goto :goto_7

    :cond_d
    const/4 v2, 0x0

    :goto_7
    invoke-virtual {v2}, Lho;->j()Lrng;

    move-result-object v1

    invoke-virtual {v1, v10, v11}, Lrng;->c(J)V

    invoke-static {v13, v8}, Lq98;->z(Ljava/lang/String;Ljava/lang/String;)V

    return v7

    :cond_e
    :goto_8
    const-string v1, "onPreExecute: taskDb.failsCount > 20, REMOVE"

    invoke-static {v13, v1}, Lq98;->z(Ljava/lang/String;Ljava/lang/String;)V

    invoke-virtual {v0}, Lpca;->g()V

    return v12

    :cond_f
    move-wide/from16 v3, v17

    goto :goto_4

    :cond_10
    if-eqz v9, :cond_11

    invoke-static {v1}, Lv50;->b(Lmq9;)Z

    move-result v1

    if-nez v1, :cond_11

    invoke-static {v13, v8}, Lq98;->z(Ljava/lang/String;Ljava/lang/String;)V

    return v7

    :cond_11
    const/4 v1, 0x1

    return v1

    :cond_12
    :goto_9
    const-string v1, "onPreExecute: message or chat not found, REMOVE"

    invoke-static {v13, v1}, Lq98;->z(Ljava/lang/String;Ljava/lang/String;)V

    return v12
.end method

.method public final f(Lukg;)V
    .locals 9

    iget-object v0, p1, Lukg;->b:Ljava/lang/String;

    iget-object v1, p0, Lgo;->c:Lho;

    const/4 v2, 0x0

    if-eqz v1, :cond_0

    goto :goto_0

    :cond_0
    move-object v1, v2

    :goto_0
    invoke-virtual {v1}, Lho;->h()Lkq9;

    move-result-object v1

    iget-wide v3, p0, Lpca;->e:J

    invoke-virtual {v1, v3, v4}, Lkq9;->n(J)Lmq9;

    move-result-object v1

    if-eqz v1, :cond_7

    iget-object v3, v1, Lmq9;->j:Luu9;

    sget-object v4, Luu9;->c:Luu9;

    if-ne v3, v4, :cond_1

    goto :goto_4

    :cond_1
    invoke-static {v0}, Lgp7;->u(Ljava/lang/String;)Z

    move-result v3

    if-nez v3, :cond_5

    const-string v3, "attachment.not.ready"

    invoke-virtual {v3, v0}, Ljava/lang/Object;->equals(Ljava/lang/Object;)Z

    move-result v3

    if-eqz v3, :cond_3

    iget-object p1, p0, Lgo;->c:Lho;

    if-eqz p1, :cond_2

    goto :goto_1

    :cond_2
    move-object p1, v2

    :goto_1
    iget-object p1, p1, Lho;->I:Lfa8;

    invoke-interface {p1}, Lfa8;->getValue()Ljava/lang/Object;

    move-result-object p1

    check-cast p1, Lv50;

    invoke-virtual {p1, v1}, Lv50;->c(Lmq9;)V

    goto :goto_3

    :cond_3
    invoke-virtual {p0}, Lpca;->g()V

    const-string v3, "errors.edit-message.send-too-many-edit"

    invoke-virtual {v3, v0}, Ljava/lang/Object;->equals(Ljava/lang/Object;)Z

    move-result v0

    if-eqz v0, :cond_5

    iget-object v0, p0, Lgo;->c:Lho;

    if-eqz v0, :cond_4

    goto :goto_2

    :cond_4
    move-object v0, v2

    :goto_2
    invoke-virtual {v0}, Lho;->b()Ln11;

    move-result-object v0

    new-instance v3, Lig5;

    iget-wide v4, p0, Lpca;->d:J

    iget-wide v6, p0, Lgo;->a:J

    move-object v8, p1

    invoke-direct/range {v3 .. v8}, Lig5;-><init>(JJLukg;)V

    invoke-virtual {v0, v3}, Ln11;->c(Ljava/lang/Object;)V

    :cond_5
    :goto_3
    iget-object p1, p0, Lgo;->c:Lho;

    if-eqz p1, :cond_6

    move-object v2, p1

    :cond_6
    invoke-virtual {v2}, Lho;->b()Ln11;

    move-result-object p1

    new-instance v2, Lleh;

    iget-wide v5, v1, Lxm0;->a:J

    const/4 v7, 0x0

    iget-wide v3, p0, Lpca;->d:J

    invoke-direct/range {v2 .. v7}, Lleh;-><init>(JJZ)V

    invoke-virtual {p1, v2}, Ln11;->c(Ljava/lang/Object;)V

    :cond_7
    :goto_4
    return-void
.end method

.method public final g()V
    .locals 11

    iget-object v0, p0, Lgo;->c:Lho;

    const/4 v1, 0x0

    if-eqz v0, :cond_0

    goto :goto_0

    :cond_0
    move-object v0, v1

    :goto_0
    invoke-virtual {v0}, Lho;->j()Lrng;

    move-result-object v0

    iget-wide v2, p0, Lgo;->a:J

    invoke-virtual {v0, v2, v3}, Lrng;->d(J)V

    iget-object v0, p0, Lgo;->c:Lho;

    if-eqz v0, :cond_1

    goto :goto_1

    :cond_1
    move-object v0, v1

    :goto_1
    invoke-virtual {v0}, Lho;->h()Lkq9;

    move-result-object v0

    iget-wide v2, p0, Lpca;->e:J

    invoke-virtual {v0, v2, v3}, Lkq9;->n(J)Lmq9;

    move-result-object v0

    if-eqz v0, :cond_4

    iget-object v2, p0, Lgo;->c:Lho;

    if-eqz v2, :cond_2

    goto :goto_2

    :cond_2
    move-object v2, v1

    :goto_2
    invoke-virtual {v2}, Lho;->h()Lkq9;

    move-result-object v2

    sget-object v3, Lrq9;->e:Lrq9;

    invoke-virtual {v2, v0, v3}, Lkq9;->s(Lmq9;Lrq9;)V

    iget-object v0, p0, Lgo;->c:Lho;

    if-eqz v0, :cond_3

    move-object v1, v0

    :cond_3
    iget-object v0, v1, Lho;->K:Lfa8;

    invoke-interface {v0}, Lfa8;->getValue()Ljava/lang/Object;

    move-result-object v0

    move-object v1, v0

    check-cast v1, Lkg5;

    iget-object v9, p0, Lpca;->i:Ljava/util/List;

    iget-boolean v10, p0, Lpca;->k:Z

    iget-wide v2, p0, Lpca;->e:J

    iget-wide v4, p0, Lpca;->d:J

    iget-object v6, p0, Lpca;->m:Ljava/lang/String;

    iget-object v7, p0, Lpca;->j:Ljava/util/List;

    iget-object v8, p0, Lpca;->h:Luu9;

    invoke-virtual/range {v1 .. v10}, Lkg5;->a(JJLjava/lang/String;Ljava/util/List;Luu9;Ljava/util/List;Z)V

    :cond_4
    return-void
.end method

.method public final getId()J
    .locals 2

    iget-wide v0, p0, Lgo;->a:J

    return-wide v0
.end method

.method public final getType()Lh4c;
    .locals 1

    sget-object v0, Lh4c;->i:Lh4c;

    return-object v0
.end method

.method public final j()[B
    .locals 3

    new-instance v0, Lru/ok/tamtam/nano/Tasks$MsgEdit;

    invoke-direct {v0}, Lru/ok/tamtam/nano/Tasks$MsgEdit;-><init>()V

    iget-wide v1, p0, Lgo;->a:J

    iput-wide v1, v0, Lru/ok/tamtam/nano/Tasks$MsgEdit;->requestId:J

    iget-wide v1, p0, Lpca;->d:J

    iput-wide v1, v0, Lru/ok/tamtam/nano/Tasks$MsgEdit;->chatId:J

    iget-wide v1, p0, Lpca;->e:J

    iput-wide v1, v0, Lru/ok/tamtam/nano/Tasks$MsgEdit;->messageId:J

    iget-wide v1, p0, Lpca;->f:J

    iput-wide v1, v0, Lru/ok/tamtam/nano/Tasks$MsgEdit;->chatServerId:J

    iget-wide v1, p0, Lpca;->g:J

    iput-wide v1, v0, Lru/ok/tamtam/nano/Tasks$MsgEdit;->messageServerId:J

    iget-object v1, p0, Lpca;->l:Ljava/lang/String;

    iput-object v1, v0, Lru/ok/tamtam/nano/Tasks$MsgEdit;->text:Ljava/lang/String;

    iget-object v1, p0, Lpca;->m:Ljava/lang/String;

    iput-object v1, v0, Lru/ok/tamtam/nano/Tasks$MsgEdit;->oldText:Ljava/lang/String;

    iget-object v1, p0, Lpca;->h:Luu9;

    iget v1, v1, Luu9;->a:I

    iput v1, v0, Lru/ok/tamtam/nano/Tasks$MsgEdit;->oldStatus:I

    iget-boolean v1, p0, Lpca;->k:Z

    iput-boolean v1, v0, Lru/ok/tamtam/nano/Tasks$MsgEdit;->editAttaches:Z

    iget-object v1, p0, Lpca;->i:Ljava/util/List;

    if-eqz v1, :cond_0

    new-instance v2, Ln50;

    invoke-direct {v2}, Ljava/lang/Object;-><init>()V

    iput-object v1, v2, Ln50;->a:Ljava/util/List;

    invoke-virtual {v2}, Ln50;->c()Lc40;

    move-result-object v1

    invoke-static {v1}, Lru/ok/tamtam/nano/a;->f(Lc40;)Lru/ok/tamtam/nano/Protos$Attaches;

    move-result-object v1

    iput-object v1, v0, Lru/ok/tamtam/nano/Tasks$MsgEdit;->oldAttaches:Lru/ok/tamtam/nano/Protos$Attaches;

    :cond_0
    iget-object v1, p0, Lpca;->j:Ljava/util/List;

    if-eqz v1, :cond_1

    invoke-static {v1}, Lxq9;->c(Ljava/util/List;)Lru/ok/tamtam/nano/Protos$MessageElements;

    move-result-object v1

    iput-object v1, v0, Lru/ok/tamtam/nano/Tasks$MsgEdit;->oldElements:Lru/ok/tamtam/nano/Protos$MessageElements;

    :cond_1
    invoke-static {v0}, Lkt9;->toByteArray(Lkt9;)[B

    move-result-object v0

    return-object v0
.end method

.method public final k()I
    .locals 1

    const v0, 0xf4240

    return v0
.end method

.method public final l()Ljlg;
    .locals 15

    iget-object v0, p0, Lgo;->c:Lho;

    const/4 v1, 0x0

    if-eqz v0, :cond_0

    goto :goto_0

    :cond_0
    move-object v0, v1

    :goto_0
    invoke-virtual {v0}, Lho;->c()Lmn2;

    move-result-object v0

    iget-wide v2, p0, Lpca;->d:J

    invoke-virtual {v0, v2, v3}, Lmn2;->P(J)Lqk2;

    move-result-object v0

    iget-object v2, p0, Lgo;->c:Lho;

    if-eqz v2, :cond_1

    goto :goto_1

    :cond_1
    move-object v2, v1

    :goto_1
    invoke-virtual {v2}, Lho;->h()Lkq9;

    move-result-object v2

    iget-wide v3, p0, Lpca;->e:J

    invoke-virtual {v2, v3, v4}, Lkq9;->n(J)Lmq9;

    move-result-object v2

    if-eqz v0, :cond_7

    if-nez v2, :cond_2

    goto :goto_4

    :cond_2
    iget-boolean v3, p0, Lpca;->k:Z

    if-eqz v3, :cond_5

    iget-object v3, v2, Lmq9;->n:Lc40;

    iget-object v4, p0, Lgo;->c:Lho;

    if-eqz v4, :cond_3

    goto :goto_2

    :cond_3
    move-object v4, v1

    :goto_2
    iget-object v4, v4, Lho;->Y:Lfa8;

    invoke-interface {v4}, Lfa8;->getValue()Ljava/lang/Object;

    move-result-object v4

    check-cast v4, Lj46;

    invoke-static {v3, v4}, Lfw8;->d(Lc40;Lj46;)Lj30;

    move-result-object v3

    if-nez v3, :cond_4

    new-instance v3, Lj30;

    invoke-direct {v3}, Ljava/util/ArrayList;-><init>()V

    :cond_4
    move-object v10, v3

    goto :goto_3

    :cond_5
    move-object v10, v1

    :goto_3
    iget-object v3, v2, Lmq9;->D:Ljava/util/List;

    if-eqz v3, :cond_6

    invoke-static {v3}, Lfw8;->s(Ljava/util/List;)Ljava/util/ArrayList;

    move-result-object v1

    :cond_6
    move-object v11, v1

    new-instance v4, Loz6;

    iget-object v0, v0, Lqk2;->b:Llo2;

    iget-wide v5, v0, Llo2;->a:J

    iget-object v12, v2, Lmq9;->G:Ld05;

    const/4 v13, 0x0

    const/16 v14, 0x40

    iget-wide v7, p0, Lpca;->g:J

    iget-object v9, p0, Lpca;->l:Ljava/lang/String;

    invoke-direct/range {v4 .. v14}, Loz6;-><init>(JJLjava/lang/String;Lj30;Ljava/util/ArrayList;Ld05;Ljava/lang/Long;I)V

    return-object v4

    :cond_7
    :goto_4
    return-object v1
.end method
