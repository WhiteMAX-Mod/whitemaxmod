.class public abstract Lone/video/player/BaseVideoPlayer;
.super Ljava/lang/Object;
.source "SourceFile"

# interfaces
.implements Lxxb;


# annotations
.annotation runtime Lkotlin/Metadata;
    d1 = {
        "\u0000\u0016\n\u0002\u0018\u0002\n\u0002\u0018\u0002\n\u0002\u0010\u000e\n\u0000\n\u0002\u0018\u0002\n\u0002\u0008\u0003\u0008&\u0018\u00002\u00020\u0001J\u0017\u0010\u0005\u001a\u00020\u00042\u0006\u0010\u0003\u001a\u00020\u0002H\u0005\u00a2\u0006\u0004\u0008\u0005\u0010\u0006\u00a8\u0006\u0007"
    }
    d2 = {
        "Lone/video/player/BaseVideoPlayer;",
        "Lxxb;",
        "",
        "event",
        "Lroh;",
        "verifyThread",
        "(Ljava/lang/String;)V",
        "one-video-player_release"
    }
    k = 0x1
    mv = {
        0x2,
        0x2,
        0x0
    }
    xi = 0x30
.end annotation


# static fields
.field public static final C:Lox;

.field public static final D:Letg;


# instance fields
.field public A:I

.field public volatile B:I

.field public final a:I

.field public final b:Ljava/lang/Thread;

.field public final c:Lw5;

.field public final d:Lm1e;

.field public final e:Lhvb;

.field public f:J

.field public g:J

.field public h:J

.field public i:Ljava/lang/String;

.field public j:Ljava/lang/String;

.field public final k:Lt07;

.field public final l:Ljava/util/concurrent/CopyOnWriteArrayList;

.field public final m:Lc17;

.field public final n:Le07;

.field public final o:Ljava/util/concurrent/CopyOnWriteArrayList;

.field public final p:Li07;

.field public q:D

.field public r:J

.field public final s:Lpq0;

.field public final t:Lg2b;

.field public u:Lknc;

.field public final v:Lpa;

.field public w:F

.field public x:F

.field public volatile y:Ljfa;

.field public z:Lone/video/player/error/OneVideoPlaybackException;


# direct methods
.method static constructor <clinit>()V
    .locals 2

    sget-object v0, Lqx;->a:Lqx;

    const-string v0, "Player"

    invoke-static {v0}, Lqx;->a(Ljava/lang/String;)Lox;

    move-result-object v0

    sput-object v0, Lone/video/player/BaseVideoPlayer;->C:Lox;

    new-instance v0, Lw5;

    const/16 v1, 0xd

    invoke-direct {v0, v1}, Lw5;-><init>(I)V

    new-instance v1, Letg;

    invoke-direct {v1, v0}, Letg;-><init>(Lv57;)V

    sput-object v1, Lone/video/player/BaseVideoPlayer;->D:Letg;

    return-void
.end method

.method public constructor <init>()V
    .locals 5

    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    sget-object v0, Lv2i;->a:Ljava/util/concurrent/atomic/AtomicInteger;

    invoke-virtual {v0}, Ljava/util/concurrent/atomic/AtomicInteger;->getAndIncrement()I

    move-result v0

    iput v0, p0, Lone/video/player/BaseVideoPlayer;->a:I

    invoke-static {}, Ljava/lang/Thread;->currentThread()Ljava/lang/Thread;

    move-result-object v0

    iput-object v0, p0, Lone/video/player/BaseVideoPlayer;->b:Ljava/lang/Thread;

    sget-object v0, Lone/video/player/BaseVideoPlayer;->C:Lox;

    invoke-static {}, Landroid/os/Looper;->myLooper()Landroid/os/Looper;

    move-result-object v1

    invoke-static {}, Landroid/os/Looper;->getMainLooper()Landroid/os/Looper;

    move-result-object v2

    invoke-static {v1, v2}, Ljz8;->f(Ljava/lang/Object;Ljava/lang/Object;)Z

    move-result v1

    new-instance v2, Lw5;

    const/16 v3, 0xe

    invoke-direct {v2, v3}, Lw5;-><init>(I)V

    const-string v3, "BaseVideoPlayer.constructor"

    invoke-virtual {v0, v1, v3, v2}, Lox;->a(ZLjava/lang/String;Lv57;)V

    new-instance v0, Lw5;

    const/16 v1, 0xf

    invoke-direct {v0, v1}, Lw5;-><init>(I)V

    iput-object v0, p0, Lone/video/player/BaseVideoPlayer;->c:Lw5;

    sget-object v0, Lone/video/player/BaseVideoPlayer;->D:Letg;

    invoke-virtual {v0}, Letg;->getValue()Ljava/lang/Object;

    move-result-object v0

    check-cast v0, Lm1e;

    iput-object v0, p0, Lone/video/player/BaseVideoPlayer;->d:Lm1e;

    new-instance v0, Lhvb;

    sget-object v1, Lkyb;->a:Ljava/util/List;

    invoke-static {}, Landroid/os/Looper;->myLooper()Landroid/os/Looper;

    move-result-object v1

    invoke-direct {v0, v1}, Lhvb;-><init>(Landroid/os/Looper;)V

    iput-object v0, p0, Lone/video/player/BaseVideoPlayer;->e:Lhvb;

    const-wide/16 v1, -0x1

    iput-wide v1, p0, Lone/video/player/BaseVideoPlayer;->f:J

    iput-wide v1, p0, Lone/video/player/BaseVideoPlayer;->g:J

    iput-wide v1, p0, Lone/video/player/BaseVideoPlayer;->h:J

    new-instance v1, Lt07;

    invoke-direct {v1}, Lt07;-><init>()V

    iput-object v1, p0, Lone/video/player/BaseVideoPlayer;->k:Lt07;

    new-instance v1, Ljava/util/concurrent/CopyOnWriteArrayList;

    invoke-direct {v1}, Ljava/util/concurrent/CopyOnWriteArrayList;-><init>()V

    iput-object v1, p0, Lone/video/player/BaseVideoPlayer;->l:Ljava/util/concurrent/CopyOnWriteArrayList;

    new-instance v1, Lc17;

    invoke-direct {v1}, Lc17;-><init>()V

    iput-object v1, p0, Lone/video/player/BaseVideoPlayer;->m:Lc17;

    new-instance v1, Le07;

    invoke-direct {v1}, Le07;-><init>()V

    iput-object v1, p0, Lone/video/player/BaseVideoPlayer;->n:Le07;

    new-instance v1, Ljava/util/concurrent/CopyOnWriteArrayList;

    invoke-direct {v1}, Ljava/util/concurrent/CopyOnWriteArrayList;-><init>()V

    iput-object v1, p0, Lone/video/player/BaseVideoPlayer;->o:Ljava/util/concurrent/CopyOnWriteArrayList;

    new-instance v1, Li07;

    invoke-direct {v1}, Li07;-><init>()V

    iput-object v1, p0, Lone/video/player/BaseVideoPlayer;->p:Li07;

    invoke-static {}, Landroid/os/SystemClock;->elapsedRealtime()J

    new-instance v1, Lpq0;

    move-object v2, p0

    check-cast v2, Ljxb;

    invoke-direct {v1, v2}, Lpq0;-><init>(Ljxb;)V

    iput-object v1, p0, Lone/video/player/BaseVideoPlayer;->s:Lpq0;

    sget-object v3, Llnc;->a:Lg2b;

    iput-object v3, p0, Lone/video/player/BaseVideoPlayer;->t:Lg2b;

    sget-object v3, Lpa;->d:Lpa;

    iput-object v3, p0, Lone/video/player/BaseVideoPlayer;->v:Lpa;

    const/high16 v3, 0x3f800000    # 1.0f

    iput v3, p0, Lone/video/player/BaseVideoPlayer;->w:F

    iput v3, p0, Lone/video/player/BaseVideoPlayer;->x:F

    const/4 v3, 0x1

    iput v3, p0, Lone/video/player/BaseVideoPlayer;->A:I

    new-instance v4, Lsq0;

    invoke-direct {v4, v2}, Lsq0;-><init>(Ljxb;)V

    const-string v2, "BaseVideoPlayer constructor"

    invoke-virtual {p0, v2}, Lone/video/player/BaseVideoPlayer;->h(Ljava/lang/String;)V

    iget-object v0, v0, Lhvb;->b:Ljava/lang/Object;

    check-cast v0, Ljava/util/LinkedHashSet;

    invoke-interface {v0, v1}, Ljava/util/Set;->add(Ljava/lang/Object;)Z

    invoke-virtual {p0, v4}, Lone/video/player/BaseVideoPlayer;->g(Luxb;)V

    sget-object v0, Lkyb;->a:Ljava/util/List;

    iput v3, p0, Lone/video/player/BaseVideoPlayer;->B:I

    return-void
.end method

.method public static t(Lone/video/player/BaseVideoPlayer;I)V
    .locals 2

    iget v0, p0, Lone/video/player/BaseVideoPlayer;->B:I

    if-eq v0, p1, :cond_0

    sget-object v0, Lkyb;->a:Ljava/util/List;

    iget v0, p0, Lone/video/player/BaseVideoPlayer;->B:I

    iput p1, p0, Lone/video/player/BaseVideoPlayer;->B:I

    const/4 v1, 0x0

    iput-object v1, p0, Lone/video/player/BaseVideoPlayer;->z:Lone/video/player/error/OneVideoPlaybackException;

    iget-object v1, p0, Lone/video/player/BaseVideoPlayer;->k:Lt07;

    invoke-virtual {v1, p0, v0, p1}, Lt07;->s(Lone/video/player/BaseVideoPlayer;II)V

    :cond_0
    return-void
.end method


# virtual methods
.method public a()Ljava/lang/String;
    .locals 2

    const-string v0, "one.video.player.BaseVideoPlayer.getDebugInfoString"

    invoke-virtual {p0, v0}, Lone/video/player/BaseVideoPlayer;->verifyThread(Ljava/lang/String;)V

    iget-object v0, p0, Lone/video/player/BaseVideoPlayer;->i:Ljava/lang/String;

    iget-object v1, p0, Lone/video/player/BaseVideoPlayer;->j:Ljava/lang/String;

    invoke-static {p0, v0, v1}, Ll8l;->a(Lone/video/player/BaseVideoPlayer;Ljava/lang/String;Ljava/lang/String;)Ljava/lang/String;

    move-result-object p0

    return-object p0
.end method

.method public final g(Luxb;)V
    .locals 1

    const-string v0, "one.video.player.BaseVideoPlayer.addListener"

    invoke-virtual {p0, v0}, Lone/video/player/BaseVideoPlayer;->verifyThread(Ljava/lang/String;)V

    iget-object p0, p0, Lone/video/player/BaseVideoPlayer;->k:Lt07;

    iget-object v0, p0, Lt07;->b:Ljava/util/concurrent/CopyOnWriteArrayList;

    invoke-virtual {v0, p1}, Ljava/util/concurrent/CopyOnWriteArrayList;->add(Ljava/lang/Object;)Z

    sget-object v0, Lkyb;->a:Ljava/util/List;

    iget-object p0, p0, Lt07;->b:Ljava/util/concurrent/CopyOnWriteArrayList;

    invoke-virtual {p0}, Ljava/util/concurrent/CopyOnWriteArrayList;->size()I

    invoke-virtual {p1}, Ljava/lang/Object;->toString()Ljava/lang/String;

    return-void
.end method

.method public final h(Ljava/lang/String;)V
    .locals 2

    const-string v0, "["

    const-string v1, "] "

    iget p0, p0, Lone/video/player/BaseVideoPlayer;->a:I

    invoke-static {p0, v0, v1, p1}, Lqh5;->j(ILjava/lang/String;Ljava/lang/String;Ljava/lang/String;)Ljava/lang/String;

    move-result-object p0

    const-string p1, "BaseVideoPlayer"

    const/4 v0, 0x0

    invoke-static {p1, p0, v0}, Landroid/util/Log;->d(Ljava/lang/String;Ljava/lang/String;Ljava/lang/Throwable;)I

    return-void
.end method

.method public i()Lmnc;
    .locals 0

    iget-object p0, p0, Lone/video/player/BaseVideoPlayer;->t:Lg2b;

    return-object p0
.end method

.method public final j()I
    .locals 1

    const-string v0, "one.video.player.BaseVideoPlayer.getState"

    invoke-virtual {p0, v0}, Lone/video/player/BaseVideoPlayer;->verifyThread(Ljava/lang/String;)V

    iget p0, p0, Lone/video/player/BaseVideoPlayer;->B:I

    return p0
.end method

.method public k()J
    .locals 2

    const-string v0, "one.video.player.BaseVideoPlayer.getVideoFrameProcessingOffsetAverage"

    invoke-virtual {p0, v0}, Lone/video/player/BaseVideoPlayer;->verifyThread(Ljava/lang/String;)V

    const-wide/16 v0, 0x64

    return-wide v0
.end method

.method public l(F)Ljava/lang/Float;
    .locals 0

    const/4 p0, 0x0

    return-object p0
.end method

.method public m(I)I
    .locals 0

    const/4 p0, 0x0

    return p0
.end method

.method public n(F)Ljava/lang/Float;
    .locals 0

    const/4 p0, 0x0

    return-object p0
.end method

.method public final o(J)V
    .locals 7

    move-object v0, p0

    check-cast v0, Ljxb;

    invoke-virtual {v0}, Ljxb;->z()Lugi;

    move-result-object v1

    const/4 v2, 0x0

    if-eqz v1, :cond_0

    invoke-virtual {v1}, Lugi;->b()Z

    move-result v1

    const/4 v3, 0x1

    if-ne v1, v3, :cond_0

    move v2, v3

    :cond_0
    invoke-virtual {v0}, Ljxb;->y()J

    move-result-wide v3

    const-string v1, "one.video.exo.OneVideoExoPlayer.getBufferedPosition"

    invoke-virtual {v0, v1}, Lone/video/player/BaseVideoPlayer;->verifyThread(Ljava/lang/String;)V

    iget-object v0, v0, Ljxb;->V:Lc76;

    invoke-virtual {v0}, Lc76;->R()J

    move-result-wide v0

    invoke-static {}, Landroid/os/SystemClock;->elapsedRealtime()J

    sget-object v5, Lkyb;->a:Ljava/util/List;

    iget-wide v5, p0, Lone/video/player/BaseVideoPlayer;->f:J

    cmp-long v5, v3, v5

    if-nez v5, :cond_1

    iget-wide v5, p0, Lone/video/player/BaseVideoPlayer;->g:J

    cmp-long v5, v0, v5

    if-nez v5, :cond_1

    if-eqz v2, :cond_4

    iget-wide v5, p0, Lone/video/player/BaseVideoPlayer;->h:J

    cmp-long v5, p1, v5

    if-eqz v5, :cond_4

    :cond_1
    iput-wide v3, p0, Lone/video/player/BaseVideoPlayer;->f:J

    iput-wide v0, p0, Lone/video/player/BaseVideoPlayer;->g:J

    iput-wide p1, p0, Lone/video/player/BaseVideoPlayer;->h:J

    const-wide/16 v0, -0x1

    cmp-long v3, v3, v0

    if-lez v3, :cond_2

    cmp-long p1, p1, v0

    if-gtz p1, :cond_3

    :cond_2
    if-eqz v2, :cond_4

    :cond_3
    iget-object p1, p0, Lone/video/player/BaseVideoPlayer;->l:Ljava/util/concurrent/CopyOnWriteArrayList;

    invoke-virtual {p1}, Ljava/util/concurrent/CopyOnWriteArrayList;->iterator()Ljava/util/Iterator;

    move-result-object p1

    :goto_0
    invoke-interface {p1}, Ljava/util/Iterator;->hasNext()Z

    move-result p2

    if-eqz p2, :cond_4

    invoke-interface {p1}, Ljava/util/Iterator;->next()Ljava/lang/Object;

    move-result-object p2

    check-cast p2, Lwxb;

    iget-wide v0, p0, Lone/video/player/BaseVideoPlayer;->f:J

    invoke-interface {p2, p0, v0, v1}, Lwxb;->a(Lone/video/player/BaseVideoPlayer;J)V

    goto :goto_0

    :cond_4
    return-void
.end method

.method public abstract p(Lknc;Lnnc;Z)V
.end method

.method public final q(Lugi;J)V
    .locals 2

    const-string v0, "one.video.player.BaseVideoPlayer.play"

    invoke-virtual {p0, v0}, Lone/video/player/BaseVideoPlayer;->verifyThread(Ljava/lang/String;)V

    sget-object v1, Lkyb;->a:Ljava/util/List;

    invoke-virtual {p1}, Ljava/lang/Object;->toString()Ljava/lang/String;

    new-instance v1, Lknc;

    invoke-static {p1}, Ljava/util/Collections;->singletonList(Ljava/lang/Object;)Ljava/util/List;

    move-result-object p1

    check-cast p1, Ljava/lang/Iterable;

    invoke-direct {v1, p1}, Lknc;-><init>(Ljava/lang/Iterable;)V

    sget-object p1, Lnnc;->d:Lnnc;

    invoke-virtual {p1, p2, p3}, Lnnc;->c(J)Lnnc;

    move-result-object p1

    invoke-virtual {p0, v0}, Lone/video/player/BaseVideoPlayer;->verifyThread(Ljava/lang/String;)V

    sget-object p2, Lkyb;->a:Ljava/util/List;

    invoke-virtual {v1}, Ljava/lang/Object;->toString()Ljava/lang/String;

    invoke-virtual {p1}, Ljava/lang/Object;->toString()Ljava/lang/String;

    invoke-virtual {p0}, Lone/video/player/BaseVideoPlayer;->i()Lmnc;

    move-result-object p2

    invoke-virtual {v1}, Lknc;->a()Ljava/util/ArrayList;

    move-result-object p3

    invoke-interface {p2, p3}, Lmnc;->a(Ljava/util/ArrayList;)Lknc;

    move-result-object p2

    iput-object p2, p0, Lone/video/player/BaseVideoPlayer;->u:Lknc;

    const/4 p3, 0x1

    invoke-virtual {p0, p2, p1, p3}, Lone/video/player/BaseVideoPlayer;->p(Lknc;Lnnc;Z)V

    return-void
.end method

.method public final r(Lknc;Lnnc;)V
    .locals 1

    const-string v0, "one.video.player.BaseVideoPlayer.prepare"

    invoke-virtual {p0, v0}, Lone/video/player/BaseVideoPlayer;->verifyThread(Ljava/lang/String;)V

    sget-object v0, Lkyb;->a:Ljava/util/List;

    invoke-virtual {p1}, Ljava/lang/Object;->toString()Ljava/lang/String;

    invoke-virtual {p2}, Ljava/lang/Object;->toString()Ljava/lang/String;

    invoke-virtual {p0}, Lone/video/player/BaseVideoPlayer;->i()Lmnc;

    move-result-object v0

    invoke-virtual {p1}, Lknc;->a()Ljava/util/ArrayList;

    move-result-object p1

    invoke-interface {v0, p1}, Lmnc;->a(Ljava/util/ArrayList;)Lknc;

    move-result-object p1

    iput-object p1, p0, Lone/video/player/BaseVideoPlayer;->u:Lknc;

    const/4 v0, 0x0

    invoke-virtual {p0, p1, p2, v0}, Lone/video/player/BaseVideoPlayer;->p(Lknc;Lnnc;Z)V

    return-void
.end method

.method public final s(Lugi;J)V
    .locals 1

    const-string v0, "one.video.player.BaseVideoPlayer.prepare"

    invoke-virtual {p0, v0}, Lone/video/player/BaseVideoPlayer;->verifyThread(Ljava/lang/String;)V

    sget-object v0, Lkyb;->a:Ljava/util/List;

    invoke-virtual {p1}, Ljava/lang/Object;->toString()Ljava/lang/String;

    new-instance v0, Lknc;

    invoke-static {p1}, Ljava/util/Collections;->singletonList(Ljava/lang/Object;)Ljava/util/List;

    move-result-object p1

    check-cast p1, Ljava/lang/Iterable;

    invoke-direct {v0, p1}, Lknc;-><init>(Ljava/lang/Iterable;)V

    sget-object p1, Lnnc;->d:Lnnc;

    invoke-virtual {p1, p2, p3}, Lnnc;->c(J)Lnnc;

    move-result-object p1

    invoke-virtual {p0, v0, p1}, Lone/video/player/BaseVideoPlayer;->r(Lknc;Lnnc;)V

    return-void
.end method

.method public final u(Ljfa;)V
    .locals 3

    const-string v0, "one.video.player.BaseVideoPlayer.setSurfaceHolder"

    invoke-virtual {p0, v0}, Lone/video/player/BaseVideoPlayer;->verifyThread(Ljava/lang/String;)V

    sget-object v0, Lkyb;->a:Ljava/util/List;

    invoke-static {p1}, Ljava/util/Objects;->toString(Ljava/lang/Object;)Ljava/lang/String;

    iget-object v0, p0, Lone/video/player/BaseVideoPlayer;->y:Ljfa;

    if-ne v0, p1, :cond_0

    return-void

    :cond_0
    iput-object p1, p0, Lone/video/player/BaseVideoPlayer;->y:Ljfa;

    const/4 v0, 0x0

    if-eqz p1, :cond_1

    invoke-virtual {p1}, Ljfa;->o()Landroid/view/Surface;

    move-result-object p1

    goto :goto_0

    :cond_1
    move-object p1, v0

    :goto_0
    if-eqz p1, :cond_4

    check-cast p0, Ljxb;

    const-string v0, "one.video.exo.OneVideoExoPlayer.setVideoSurface"

    invoke-virtual {p0, v0}, Lone/video/player/BaseVideoPlayer;->verifyThread(Ljava/lang/String;)V

    new-instance v0, Lcta;

    const/16 v1, 0xa

    invoke-direct {v0, p1, v1}, Lcta;-><init>(Ljava/lang/Object;I)V

    iget-object v1, p0, Ljxb;->G:Lbjb;

    sget-object v2, Lkyb;->a:Ljava/util/List;

    invoke-virtual {v0}, Lcta;->invoke()Ljava/lang/Object;

    if-eqz v1, :cond_2

    invoke-virtual {v1}, Lbjb;->invoke()Ljava/lang/Object;

    :cond_2
    iget-object v0, p0, Lone/video/player/BaseVideoPlayer;->d:Lm1e;

    if-eqz v0, :cond_3

    invoke-virtual {v0, p0, p1}, Lm1e;->g(Ljava/lang/Object;Landroid/view/Surface;)V

    return-void

    :cond_3
    iget-object p0, p0, Ljxb;->V:Lc76;

    invoke-virtual {p0, p1}, Lc76;->C0(Landroid/view/Surface;)V

    return-void

    :cond_4
    check-cast p0, Ljxb;

    const-string p1, "one.video.exo.OneVideoExoPlayer.clearVideoSurface"

    invoke-virtual {p0, p1}, Lone/video/player/BaseVideoPlayer;->verifyThread(Ljava/lang/String;)V

    iget-object p1, p0, Ljxb;->G:Lbjb;

    invoke-static {p1}, Ljxb;->w(Lv57;)V

    iget-object p1, p0, Lone/video/player/BaseVideoPlayer;->d:Lm1e;

    if-eqz p1, :cond_5

    invoke-virtual {p1, p0, v0}, Lm1e;->g(Ljava/lang/Object;Landroid/view/Surface;)V

    return-void

    :cond_5
    iget-object p0, p0, Ljxb;->V:Lc76;

    invoke-virtual {p0}, Lc76;->P()V

    return-void
.end method

.method public final verifyThread(Ljava/lang/String;)V
    .locals 4

    sget-object v0, Lkyb;->a:Ljava/util/List;

    invoke-static {}, Ljava/lang/Thread;->currentThread()Ljava/lang/Thread;

    move-result-object v0

    iget-object v1, p0, Lone/video/player/BaseVideoPlayer;->b:Ljava/lang/Thread;

    if-ne v1, v0, :cond_0

    const/4 v1, 0x1

    goto :goto_0

    :cond_0
    const/4 v1, 0x0

    :goto_0
    new-instance v2, Ls5;

    const/4 v3, 0x5

    invoke-direct {v2, v3, v0, p0}, Ls5;-><init>(ILjava/lang/Object;Ljava/lang/Object;)V

    sget-object p0, Lone/video/player/BaseVideoPlayer;->C:Lox;

    invoke-virtual {p0, v1, p1, v2}, Lox;->a(ZLjava/lang/String;Lv57;)V

    return-void
.end method
