.class public final Lbk1;
.super Lalf;
.source "SourceFile"


# instance fields
.field public final X:Ljava/util/concurrent/Executor;

.field public final o:Ll17;


# direct methods
.method public constructor <init>(Ll17;)V
    .locals 1

    sget-object v0, Lsme;->a:Lsme;

    invoke-virtual {v0}, Lsme;->d()Lncb;

    move-result-object v0

    invoke-virtual {v0}, Lncb;->a()Ljava/util/concurrent/ExecutorService;

    move-result-object v0

    invoke-direct {p0, v0}, Lalf;-><init>(Ljava/util/concurrent/Executor;)V

    iput-object p1, p0, Lbk1;->o:Ll17;

    iput-object v0, p0, Lbk1;->X:Ljava/util/concurrent/Executor;

    return-void
.end method


# virtual methods
.method public final l(I)I
    .locals 1

    iget-object v0, p0, Lfg8;->d:Lcy;

    iget-object v0, v0, Lcy;->f:Ljava/util/List;

    invoke-interface {v0, p1}, Ljava/util/List;->get(I)Ljava/lang/Object;

    move-result-object p1

    check-cast p1, Lmg8;

    invoke-interface {p1}, Lmg8;->m()I

    move-result p1

    return p1
.end method

.method public final t(Lpyd;ILjava/util/List;)V
    .locals 5

    check-cast p1, Lhmf;

    invoke-interface {p3}, Ljava/util/List;->isEmpty()Z

    move-result v0

    iget-object v1, p0, Lfg8;->d:Lcy;

    if-eqz v0, :cond_0

    iget-object p3, v1, Lcy;->f:Ljava/util/List;

    invoke-interface {p3, p2}, Ljava/util/List;->get(I)Ljava/lang/Object;

    move-result-object p2

    check-cast p2, Lmg8;

    invoke-virtual {p1, p2}, Lhmf;->y(Lmg8;)V

    return-void

    :cond_0
    iget-object v0, v1, Lcy;->f:Ljava/util/List;

    invoke-interface {v0, p2}, Ljava/util/List;->get(I)Ljava/lang/Object;

    move-result-object v0

    check-cast v0, Lmg8;

    invoke-interface {v0}, Lmg8;->m()I

    move-result v0

    const/4 v2, 0x1

    if-ne v0, v2, :cond_8

    check-cast p1, Lak1;

    iget-object p2, p1, Lpyd;->a:Landroid/view/View;

    new-instance v0, Lpu;

    const/4 v1, 0x2

    invoke-direct {v0, v1, p3}, Lpu;-><init>(ILjava/lang/Object;)V

    new-instance p3, Lnc1;

    const/16 v1, 0x9

    invoke-direct {p3, v1}, Lnc1;-><init>(I)V

    new-instance v1, Le86;

    sget-object v2, Lvwe;->a:Lvwe;

    const/4 v3, 0x0

    invoke-direct {v1, v0, p3, v2, v3}, Le86;-><init>(Ljava/lang/Object;Lks6;Lks6;I)V

    sget-object p3, La51;->A0:La51;

    invoke-static {v1, p3}, Lswe;->e(Lgwe;Lks6;)Ln56;

    move-result-object p3

    new-instance v0, Lm56;

    invoke-direct {v0, p3}, Lm56;-><init>(Ln56;)V

    :goto_0
    invoke-virtual {v0}, Lm56;->hasNext()Z

    move-result p3

    if-eqz p3, :cond_7

    invoke-virtual {v0}, Lm56;->next()Ljava/lang/Object;

    move-result-object p3

    check-cast p3, Luj1;

    instance-of v1, p3, Ltj1;

    if-eqz v1, :cond_1

    move-object v1, p2

    check-cast v1, Lf9b;

    check-cast p3, Ltj1;

    iget-object p3, p3, Ltj1;->a:Ljava/lang/String;

    invoke-virtual {v1, p3}, Lf9b;->setTitle(Ljava/lang/CharSequence;)V

    goto :goto_0

    :cond_1
    instance-of v1, p3, Lqj1;

    if-eqz v1, :cond_2

    move-object v1, p2

    check-cast v1, Lf9b;

    check-cast p3, Lqj1;

    iget-object p3, p3, Lqj1;->a:Ljava/lang/Integer;

    invoke-virtual {p3}, Ljava/lang/Number;->intValue()I

    move-result p3

    invoke-virtual {v1}, Landroid/view/View;->getContext()Landroid/content/Context;

    move-result-object v2

    invoke-virtual {v2, p3}, Landroid/content/Context;->getString(I)Ljava/lang/String;

    move-result-object p3

    invoke-virtual {v1, p3}, Lf9b;->setSubtitle(Ljava/lang/CharSequence;)V

    goto :goto_0

    :cond_2
    instance-of v1, p3, Lpj1;

    if-eqz v1, :cond_3

    move-object v1, p2

    check-cast v1, Lf9b;

    check-cast p3, Lpj1;

    iget-object v2, p3, Lpj1;->a:Lpl1;

    iget-wide v2, v2, Lpl1;->a:J

    iget-object v4, p3, Lpj1;->b:Ljava/lang/String;

    iget-object p3, p3, Lpj1;->c:Ljava/lang/String;

    invoke-virtual {v1, v2, v3, v4, p3}, Lf9b;->g(JLjava/lang/CharSequence;Ljava/lang/String;)V

    goto :goto_0

    :cond_3
    instance-of v1, p3, Lrj1;

    if-eqz v1, :cond_4

    check-cast p3, Lrj1;

    iget-object v1, p3, Lrj1;->a:Lpl1;

    iget-boolean v2, p3, Lrj1;->b:Z

    iget-boolean p3, p3, Lrj1;->c:Z

    invoke-virtual {p1, v1, v2, p3}, Lak1;->E(Lpl1;ZZ)V

    goto :goto_0

    :cond_4
    instance-of v1, p3, Lsj1;

    if-eqz v1, :cond_6

    check-cast p3, Lsj1;

    iget-boolean v1, p3, Lsj1;->a:Z

    iget-object p3, p3, Lsj1;->b:Lpl1;

    if-eqz v1, :cond_5

    const/4 p3, 0x0

    invoke-virtual {p2, p3}, Landroid/view/View;->setOnClickListener(Landroid/view/View$OnClickListener;)V

    goto :goto_0

    :cond_5
    new-instance v1, Lld;

    const/4 v2, 0x7

    invoke-direct {v1, p1, v2, p3}, Lld;-><init>(Ljava/lang/Object;ILjava/lang/Object;)V

    invoke-static {p2, v1}, Lwuj;->c(Landroid/view/View;Landroid/view/View$OnClickListener;)V

    goto :goto_0

    :cond_6
    new-instance p1, Lkotlin/NoWhenBranchMatchedException;

    invoke-direct {p1}, Lkotlin/NoWhenBranchMatchedException;-><init>()V

    throw p1

    :cond_7
    return-void

    :cond_8
    iget-object p3, v1, Lcy;->f:Ljava/util/List;

    invoke-interface {p3, p2}, Ljava/util/List;->get(I)Ljava/lang/Object;

    move-result-object p2

    check-cast p2, Lmg8;

    invoke-virtual {p1, p2}, Lhmf;->y(Lmg8;)V

    return-void
.end method

.method public final u(Landroid/view/ViewGroup;I)Lpyd;
    .locals 2

    const/4 v0, 0x1

    if-ne p2, v0, :cond_0

    new-instance p2, Lak1;

    invoke-virtual {p1}, Landroid/view/View;->getContext()Landroid/content/Context;

    move-result-object p1

    iget-object v0, p0, Lbk1;->o:Ll17;

    invoke-direct {p2, p1, v0}, Lak1;-><init>(Landroid/content/Context;Ll17;)V

    return-object p2

    :cond_0
    new-instance p1, Ljava/lang/IllegalArgumentException;

    const-string v0, "Not supported viewType="

    const-string v1, " for CallOpponentsListAdapter"

    invoke-static {p2, v0, v1}, Lau1;->g(ILjava/lang/String;Ljava/lang/String;)Ljava/lang/String;

    move-result-object p2

    invoke-direct {p1, p2}, Ljava/lang/IllegalArgumentException;-><init>(Ljava/lang/String;)V

    throw p1
.end method
